...
export default {
  ...
  methods: {
    ...
    async GetMe(authObj) {
      ...
      window.Kakao.API.request({
        ...
        success: (res) => {
          ...
          axios
            .post(`${this.constants.API_URL}rest-auth/kakao/login/`, form)
            .then((res) => {
    ...
    async isFirstLogin() {
      ...
      axios
        .get(`${this.constants.API_URL}rest-auth/user/`, {
          headers: {
            Authorization: Token,
          },
        })
    ...
    login() {
      axios
        .post(`${this.constants.API_URL}rest-auth/login/`, this.loginData)
        .then((res) => {
          this.SET_TOKEN(res.data.access_token);
          this.SET_USERNAME(res.data.user.username);
          this.isFirstLogin();
    ...
    submit() {
      axios
        .post(`${this.constants.API_URL}rest-auth/signup/`, this.submitData)
        .then((res) => {
          this.SET_TOKEN(res.data.access_token);
          this.SET_USERNAME(res.data.user.username);
          this.isFirstLogin();
